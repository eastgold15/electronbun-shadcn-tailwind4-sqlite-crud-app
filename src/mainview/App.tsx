          ...
          <div className="flex gap-2 mb-4">
            {(["all", "active", "completed"] as FilterType[]).map((f) => (
              <button
                key={f}
                onClick={() => setFilter(f)}
                className={`flex-1 px-3 py-2 rounded-lg font-medium transition-colors text-sm ${filter === f
          ...
          <ul id="todo-list" className="space-y-2">
            {filteredTodos.length === 0 ? (
              ...
              filteredTodos.map((todo) => (
                <li
                  key={todo.id}
                  className={`todo-item flex items-center gap-3 p-3 bg-gray-50 rounded-lg group ${todo.completed ? "completed" : ""
                    }`}
                >
                  <input
                    type="checkbox"
                  ...
                  />
                  <span
                    className={`flex-1 todo-text text-sm ${todo.completed ? "line-through text-gray-400" : "text-gray-700"
                      }`}
                  >
                    {todo.title}
                  </span>
